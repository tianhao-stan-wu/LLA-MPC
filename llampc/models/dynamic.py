...
import numpy as np
import casadi as cs
from llampc.models.model import Model
...
class Dynamic(Model):
	...
	def casadi(self, x, u, dxdt):
		...
		dxdt[2] = omega
		dxdt[3] = 1/self.mass * (Frx - Ffy*cs.sin(steer)) + vy*omega #-9.8*cs.sin(pitch)
		dxdt[4] = 1/self.mass * (Fry + Ffy*cs.cos(steer)) - vx*omega #-9.8*cs.sin(roll)
	...
	def casadi_parametric(self, x, u, dxdt, params):
		...
		# Extract parameters with bounds for numerical stability
		Bf = cs.fmax(0.05, params[0])  # prevent negative/zero stiffness
		...
		# Tire forces with parametric coefficients
	...
	# def casadi_parametric(self, x, u, dxdt, params):
	# 	"""Same as casadi() but with symbolic parameters
    #        params is a 6x1 vector: [Bf, Cf, Df, Br, Cr, Dr]
    #     """
	# 	# Unpack model parameters
	# 	Bf, Cf, Df, Br, Cr, Dr = params[0], params[1], params[2], params[3], params[4], params[5]
	#
	#
