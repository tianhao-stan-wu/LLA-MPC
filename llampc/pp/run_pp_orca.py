...
from llampc.pp import purePursuit
...
ObjectS, = ax.plot(states[0,0] + dims[:,0]*np.cos(states[2,0]) - dims[:,1]*np.sin(states[2,0])\
...
for idt in range(n_steps):
	...
	start = tm.time()
	upp, ind = purePursuit(x0, LD, KP, track, params)
	ref_speeds.append(track.v_raceline[ind])
	end = tm.time()
	inputs[:,idt] = upp
	...
	ObjectS.set_xdata(states[0,idt] + dims[:,0]*np.cos(states[2,idt]) - dims[:,1]*np.sin(states[2,idt]))
	ObjectS.set_ydata(states[1,idt] + dims[:,0]*np.sin(states[2,idt]) + dims[:,1]*np.cos(states[2,idt]))
...
plt.plot(time[:n_steps], ref_speeds, label='ref')
plt.xlabel('time [s]')
