...
def load_data(CTYPE, TRACK_NAME, VARIDX, xscaler=None, yscaler=None):
	...
	x = np.concatenate([
		...
		data_dyn['states'][5:6,:N_SAMPLES].T*data_dyn['states'][3:4,:N_SAMPLES].T,
		data_dyn['states'][6:7,:N_SAMPLES].T*data_dyn['states'][3:4,:N_SAMPLES].T,
		data_kin['inputs'][:,:N_SAMPLES].T*data_dyn['states'][3:4,:N_SAMPLES].T,
		data_dyn['states'][3:4,:N_SAMPLES].T**2,
		data_dyn['states'][3:4,:N_SAMPLES].T,
...
for VARIDX in VARIDXs :
	...
	# print(kernel)
	if first :
		model = make_pipeline(StandardScaler(),SGDRegressor(max_iter=1000, tol=1e-5))
		# first = False
	else :
