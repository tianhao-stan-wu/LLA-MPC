...
def odeintRK6_batch(fun, y0_batch, t, args_batch):
    """Batched version of odeintRK6"""
    ...
    for i in range(len(t)-1):
        h = t[i+1]-t[i]
        k1 = h*fun(t[i], y0_batch, *args_batch)
        k2 = h*fun(t[i]+h/4, y0_batch+k1/4, *args_batch)
        k3 = h*fun(t[i]+3/8*h, y0_batch+3/32*k1+9/32*k2, *args_batch)
        k4 = h*fun(t[i]+12/13*h, y0_batch+1932/2197*k1-7200/2197*k2+7296/2197*k3, *args_batch)
        k5 = h*fun(t[i]+h, y0_batch+439/216*k1-8*k2+3680/513*k3-845/4104*k4, *args_batch)
        k6 = h*fun(t[i]+h/2, y0_batch-8/27*k1+2*k2-3544/2565*k3+1859/4104*k4-11/40*k5, *args_batch)
...
def odeintRK4_batch(fun, y0_batch, t, args_batch):
    ...
    for i in range(len(t) - 1):
        ...
        # RK4 steps
        k1 = h * fun(t[i], y0_batch, *args_batch)
        k2 = h * fun(t[i] + h / 2, y0_batch + k1 / 2, *args_batch)
        k3 = h * fun(t[i] + h / 2, y0_batch + k2 / 2, *args_batch)
        k4 = h * fun(t[i] + h, y0_batch + k3, *args_batch)
...
def odeintEuler_batch(fun, y0_batch, t, args_batch):
    """Batched version of Euler integration"""
    n_models = y0_batch.shape[0]
    y_next = np.zeros([len(t) - 1, n_models, y0_batch.shape[1]])
    ...
    for i in range(len(t) - 1):
        ...
        # Single Euler step
        k = h * fun(t[i], y0_batch, *args_batch)
