...
from llampc.utils import odeintRK6
from llampc.params import ORCA, F110
from llampc.tracks import ETHZ, ETHZMobil, UCB
import math
...
def define_params(mass, lf, lr):
    ...
    # Dr = 0.1737
    # Df = 0.192
    muf = 1.#(Dr+Df)/(gravity*mass)                 # coefficient of friction
...
def plots(t, x, y, vxy, u, S, params):
    ...
    plt.plot(t,v_raceline, label='gt speeds')
...
def simulate(b, a, u, path, params, plot_results, print_updates, int_method ='rk6'):
    ...
    vx, vy = np.sqrt(b)*params['Flongmax']*4.2/(6.*141.*params['mass']*dtheta), np.zeros([num_wpts])
    # vx[0] = 0.
